...
func TestGetVMsFromCache(t *testing.T) {
	...
	ac, err := newAzureCache(testAS.manager.azClient, refreshInterval, testAS.manager.config.ResourceGroup, providerazureconsts.VMTypeStandard, false, "")
	assert.NoError(t, err)
...
func TestGetVMIndexes(t *testing.T) {
	...
	ac, err := newAzureCache(as.manager.azClient, refreshInterval, as.manager.config.ResourceGroup, providerazureconsts.VMTypeStandard, false, "")
	assert.NoError(t, err)
